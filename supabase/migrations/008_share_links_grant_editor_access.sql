-- Share links should let recipients edit documents, not only view them.
-- This migration replaces the live RPC even when earlier migrations already ran.
...
BEGIN
  ...
  ON CONFLICT ON CONSTRAINT document_collaborators_document_id_user_id_key DO UPDATE
    SET role = 'editor';
...
-- Fix users who already joined via the previous read-only share-link behavior.
